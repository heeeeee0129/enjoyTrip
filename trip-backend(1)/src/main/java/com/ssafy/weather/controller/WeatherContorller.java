...
public class WeatherContorller {
	...
	private String key = "20d079fda51265996b99ca33ba5025c2";

	@Operation(summary = "해당 지역 날씨 정보 반환")
	@PostMapping("/weather")
	public ResponseEntity<?> getWeather(@RequestBody Weather weather) {
		try {
			String APIUrl = "https://api.openweathermap.org/data/2.5/weather?lat=" + weather.getLat() + "&lon=" + weather.getLon() + "&appid=" + key;
	        ...
	        String sunrise = new java.text.SimpleDateFormat("HH:mm:ss").format(new java.util.Date(jsonResponse.getJSONObject("sys").getLong("sunrise") * 1000));
	        String sunset = new java.text.SimpleDateFormat("HH:mm:ss").format(new java.util.Date(jsonResponse.getJSONObject("sys").getLong("sunset") * 1000));
