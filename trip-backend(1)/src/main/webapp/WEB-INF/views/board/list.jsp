...
<body class="medium">
	...
	<div class="container board-container">
		<h1 style="text-align: left; margin: 30px 120px;">공유게시판</h1>
		<div class="row">
			<div class="col-md-10 offset-md-1">
				<div class="flex row">
				    <div class="col-md-2 text-start mb-3">
				        <button type="button" id="btn-mv-register" class="btn btn-outline-primary">글쓰기</button>
				    </div>
				    <div class="col-md-8"> </div>
				    <div class="col-md-2 text-end mb-3">
					    <select class="form-select" id="sort-select">
					        <option value="register" ${sessionScope.selectedSort == 'register' ? 'selected' : ''}>최신순</option>
					        <option value="popularity" ${sessionScope.selectedSort == 'popularity' ? 'selected' : ''}>인기순</option>
					        <option value="alphabetical" ${sessionScope.selectedSort == 'alphabetical' ? 'selected' : ''}>가나다순</option>
					    </select>
					</div>
				</div>
	...
	document.querySelector("#sort-select").addEventListener("change", function() {
	    ...
	    let url = "${root}/article?action=sort&sortby=" + sortBy;
